...
public class Transaction {
	...
	public boolean processTransaction() {
			...
			for(TransactionInput i : inputs) {
				i.UTXO = Vacchain.UTXOs.get(i.transactionOutputId);
			}
			//check if transaction is valid:
			if(getInputsValue() < Vacchain.minimumTransaction) {
			...
			if (leftOver > 0f) outputs.add(new TransactionOutput( this.sender, leftOver,transactionId)); //send the left over 'change' back to sender		
					
			//add outputs to Unspent list
			for(TransactionOutput o : outputs) {
				Vacchain.UTXOs.put(o.id , o);
			}
			
			//remove transaction inputs from UTXO lists as spent:
			for(TransactionInput i : inputs) {
				if(i.UTXO == null) continue; //if Transaction can't be found skip it 
				Vacchain.UTXOs.remove(i.UTXO.id);
			}
